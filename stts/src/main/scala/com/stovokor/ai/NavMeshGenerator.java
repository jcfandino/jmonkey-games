...
public class NavMeshGenerator implements Savable {
    ...
    public Mesh optimize(final Mesh mesh) {
        nmgen =
                new NavmeshGenerator(cellSize, cellHeight, minTraversableHeight,
                        maxTraversableStep, maxTraversableSlope, clipLedges,
                        traversableAreaBorderSize, smoothingThreshold, useConservativeExpansion,
                        minUnconnectedRegionSize, mergeRegionSize, maxEdgeLength, edgeMaxDeviation,
                        maxVertsPerPoly, contourSampleDistance, contourMaxDeviation);
    ...
    private TriangleMesh buildNavMesh(final float[] positions, final int[] indices,
        ...
        try {
            ...
            // NotifyDescriptor.Message("NavMesh Generation timed out."));
            System.out.println("Timeout generating NavMesh");
            throw new RuntimeException("NavMeshGen Timeout", ex);
        }
        return runnable.getTriMesh();
    }

    private static void execute(final Thread task, final long timeout) throws TimeoutException {
        task.start();
        try {
            task.join(timeout);
    ...
    private static void execute(final Runnable task, final long timeout) throws TimeoutException {
    ...
    /**
     * @param cellHeight
     *            - The height resolution used when sampling the source mesh. Value must be > 0.
     *            Constraints: > 0
     */
    ...
    /**
     * @param cellSize
     *            - The width and depth resolution used when sampling the the source mesh.
     *            Constraints: > 0
     */
    ...
    /**
     * Indicates whether ledges should be marked as unwalkable. A ledge is a normally walkable voxel
     * that has one or more accessible neighbors with a an un-steppable drop from voxel top to voxel
     * top. E.g. If an agent using the navmesh were to travel down from the ledge voxel to its
     * neighbor voxel, it would result in the maximum traversable step distance being violated. The
     * agent cannot legally "step down" from a ledge to its neighbor.
     */
    ...
    /**
     * The maximum distance the surface of the navmesh may deviate from the surface of the original
     * geometry. The accuracy of the algorithm which uses this value is impacted by the value of the
     * contour sample distance argument. The value of this argument has no meaning if the contour
     * sample distance argument is set to zero. Setting the value to zero is not recommended since
     * it can result in a large increase in the number of triangles in the final navmesh at a high
     * processing cost. Constraints: >= 0
     */
    ...
    /**
     * Sets the sampling distance to use when matching the navmesh to the surface of the original
     * geometry. Impacts how well the final mesh conforms to the original geometry's surface
     * contour. Higher values result in a navmesh which conforms more closely to the original
     * geometry's surface at the cost of a higher final triangle count and higher processing cost.
     * Setting this argument to zero will disable this functionality. Constraints: >= 0
     */
    ...
    /**
     * The maximum distance the edge of the navmesh may deviate from the source geometry. Setting
     * this lower will result in the navmesh edges following the geometry contour more accurately at
     * the expense of an increased triangle count. Setting the value to zero is not recommended
     * since it can result in a large increase in the number of triangles in the final navmesh at a
     * high processing cost. Constraints: >= 0
     */
    ...
    /**
     * The maximum length of polygon edges that represent the border of the navmesh. More vertices
     * will be added to navmesh border edges if this value is exceeded for a particular edge. In
     * certain cases this will reduce the number of thin, long triangles in the navmesh. A value of
     * zero will disable this feature. Constraints: >= 0
     */
    ...
    /**
     * The maximum slope that is considered walkable. (Degrees) Constraints: 0 <= value <= 85
     */
    ...
    /**
     * Represents the maximum ledge height that is considered to still be walkable. Prevents minor
     * deviations in height from improperly showing as obstructions. Permits detection of stair-like
     * structures, curbs, etc. Constraints: >= 0
     */
    ...
    /**
     * The maximum number of vertices per polygon for polygons generated during the voxel to polygon
     * conversion stage. Higher values reduce performance, but can also result in better formed
     * triangles in the navmesh. A value of around 6 is generally adequate with diminishing returns
     * for values higher than 6. Contraints: >= 3
     */
     ...
     * Helps reduce the number of unnecessarily small regions that can be formed. This is especially
     * an issue in diagonal path regions where inherent faults in the region generation algorithm
     * can result in unnecessarily small regions. If a region cannot be legally merged with a
     * neighbor region, then it will be left alone. Constraints: >= 0
     */
    ...
    /**
     * Represents the minimum floor to ceiling height that will still allow the floor area to be
     * considered walkable. Permits detection of overhangs in the geometry which make the geometry
     * below become unwalkable. Constraints: > 0
     */
    ...
    /**
     * The minimum region size for unconnected (island) regions. (Voxels) Any generated regions that
     * are not connected to any other region and are smaller than this size will be culled before
     * final navmesh generation. I.e. No longer considered walkable. Constraints: > 0
     */
    ...
    /**
     * The amount of smoothing to be performed when generating the distance field. This value
     * impacts region formation and border detection. A higher value results in generally larger
     * regions and larger border sizes. A value of zero will disable smoothing. Constraints: 0 <=
     * value <= 4
     */
     ...
     * Usually set to the maximum bounding radius of entities utilizing the navmesh for navigation
     * decisions. Constraints: >= 0
     */
    public void setTraversableAreaBorderSize(final float traversableAreaBorderSize) {
    ...
    /**
     * Applies extra algorithms to regions to help prevent poorly formed regions from forming. If
     * the navigation mesh is missing sections that should be present, then enabling this feature
     * will likely fix the problem. Enabling this feature significantly increased processing cost.
     */
    public void setUseConservativeExpansion(final boolean useConservativeExpansion) {
